    ...
    {menu && <nav className="landing-menu" data-testid="landing-menu">{[['/terminal', 'The terminal'], ['/terminal/launch', 'Launchpads'], ['/terminal/whitepaper', 'Whitepaper']].map(([path, title]) => <Link data-testid={`landing-menu-${title.replace(/\s/g, '-').toLowerCase()}`} to={path} key={path}>{title}<ArrowUpRight size={15} /></Link>)}</nav>}
     <main className={`globe-layout ${ecosystem ? 'with-panel is-focused' : ''}`}><div className="globe-main"><div className="globe-stage"><div className="landing-copy"><span className="eyebrow"><span className="live-dot" /> ONE WORLD. ALL THE OPPORTUNITY.</span><h1>Find the meta.<br />Follow the flow.<br /><span>Stay early.</span></h1><p>Every ecosystem. Your favourite launchpads.<br />One connected degen universe.</p><Link className="btn-primary globe-terminal-cta" to="/terminal" data-testid="enter-terminal">Enter the terminal<ArrowUpRight size={17} /></Link><Link className="landing-whitepaper" to="/terminal/whitepaper" data-testid="landing-whitepaper">Read the whitepaper<ArrowRight size={13} /></Link><div className="globe-legend"><span><i className="legend-chain" />ECOSYSTEM</span><span><i className="legend-launch" />LAUNCHPAD</span></div></div><div className="landing-globe"><Globe3D onSelect={openWorld} selectedId={selected} size={680} /><div className="globe-coordinate-label">GLOBAL ON-CHAIN NETWORK<span>14 CONNECTED NODES</span></div></div></div>
         <section className="node-directory"><div className="node-directory-heading"><div className="node-group-switch"><button data-testid="globe-filter-launchpads" onClick={() => setGroup('launchpads')} className={group === 'launchpads' ? 'active' : ''}><Rocket size={14} />Launchpads</button><button data-testid="globe-filter-ecosystems" onClick={() => setGroup('ecosystems')} className={group === 'ecosystems' ? 'active' : ''}><Globe2 size={14} />Ecosystems</button></div><Link to="/terminal/launch" data-testid="globe-launchpads-directory">Open directory<ArrowUpRight size={13} /></Link></div><div className="node-chips">{(group === 'launchpads' ? LAUNCHPADS : ECOSYSTEMS.filter(e => !e.isFeeless)).map(e => <button data-testid={`globe-node-${e.id}`} key={e.id} onClick={() => openWorld(e.id)} className={selected === e.id ? 'active' : ''} style={{ '--node-color': e.color }}><LaunchpadLogo launchpad={e} size={30} /><span>{e.name}<small>{group === 'launchpads' ? e.chainId === 'bsc' ? 'BNB Chain' : 'Solana' : 'Ecosystem'}</small></span><ArrowUpRight size={12} /></button>)}</div><div className="globe-bottom-note"><span>REAL MARKETS. REAL COMMUNITY. NO MADE-UP ALPHA.</span><span>Explore freely. Verify independently.</span></div></section>
          </div>{ecosystem && <aside className="degen-panel" data-testid="degen-panel" aria-label={`${ecosystem.name} ecosystem focus panel`}><div className="degen-panel-title"><span style={{ background: ecosystem.color }} /><div><small>CONNECTED TO</small><h2 data-testid="degen-panel-name">{ecosystem.name}</h2></div><button title="Close ecosystem panel" data-testid="degen-panel-close" onClick={() => setSelected(null)}><X size={18} /></button></div><button type="button" className="btn-outline degen-world-open" data-testid="degen-world-open" onClick={() => setWorldOpen(true)}><Globe2 size={15} />Open {ecosystem.name} network<ArrowUpRight size={15} /></button><Link className="btn-primary globe-terminal-cta degen-enter-context" data-testid="degen-enter-terminal" to="/terminal">Enter {ecosystem.name} command<ArrowUpRight size={15} /></Link>{pad && pad.url && <a data-testid="degen-launchpad-open" className="btn-outline" href={pad.url} target={pad.url.startsWith('/') ? '_self' : '_blank'} rel="noreferrer"><Rocket size={15} />Open {pad.name}<ArrowUpRight size={15} /></a>}<div className="degen-chat"><EcosystemChat key={ecosystem.id} ecosystem={{ ...ecosystem, id: `${ecosystem.id}-general` }} /></div>{(ecosystem.id === 'feeless' || ecosystem.isFeelessLaunch) && <FeelessCoinCards /> }<div className={`degen-coins-rail ${radarIsOpen ? 'is-open' : ''}`} data-testid="degen-coins-rail"><button type="button" className="degen-coins-tab" data-testid="degen-coins-toggle" aria-controls={`degen-coins-popover-${ecosystem.id}`} aria-expanded={radarIsOpen} aria-label={`${radarIsOpen ? 'Close' : 'Open'} coin radar feeds`} onClick={toggleRadar} onFocus={() => setRadarOpen(true)}><b>COIN RADAR</b><small>{radarIsOpen ? 'Close feeds' : 'Open feeds'}</small></button><div className="degen-coins-popover" id={`degen-coins-popover-${ecosystem.id}`}><div className="degen-coins"><TopCoins key={ecosystem.id} ecosystem={ecosystem} /></div></div></div><EcosystemPlatforms ecosystem={ecosystem} /></aside>}</main>{worldOpen && ecosystem && <EcosystemWorld ecosystem={ecosystem} pad={pad} onClose={() => setWorldOpen(false)} />}<WalletModal open={walletOpen} onClose={() => setWalletOpen(false)} />
  </div>;
}
