...
export default function Landing() {
  ...
  return <div className="globe-page"><MouseGlow /><TerminalHeader onWallet={() => setWalletOpen(true)} onMenu={() => setMenu(v => !v)} /><MarketTicker /><ContextBar />
    ...
    <main className={`globe-layout ${ecosystem ? 'with-panel' : ''}`}><div className="globe-main"><div className="globe-stage"><div className="landing-copy"><span className="eyebrow"><span className="live-dot" /> ONE WORLD. ALL THE OPPORTUNITY.</span><h1>Find the meta.<br />Follow the flow.<br /><span>Stay early.</span></h1><p>Every ecosystem. Your favourite launchpads.<br />One connected degen universe.</p><Link className="btn-primary" to="/terminal" data-testid="enter-terminal">Enter the terminal<ArrowUpRight size={17} /></Link><Link className="landing-whitepaper" to="/terminal/whitepaper" data-testid="landing-whitepaper">Read the whitepaper<ArrowRight size={13} /></Link><div className="globe-legend"><span><i className="legend-chain" />ECOSYSTEM</span><span><i className="legend-launch" />LAUNCHPAD</span></div></div><div className="landing-globe"><Globe3D onSelect={setSelected} selectedId={selected} size={680} /><div className="globe-coordinate-label">GLOBAL ON-CHAIN NETWORK<span>14 CONNECTED NODES</span></div></div></div>
      ...
      </div>{ecosystem && <aside className="degen-panel" data-testid="degen-panel"><div className="degen-panel-title"><span style={{ background: ecosystem.color }} /><div><small>CONNECTED TO</small><h2 data-testid="degen-panel-name">{ecosystem.name}</h2></div><button title="Close ecosystem panel" data-testid="degen-panel-close" onClick={() => setSelected(null)}><X size={18} /></button></div><Link className="btn-primary degen-enter-context" data-testid="degen-enter-terminal" to="/terminal">Enter {ecosystem.name} command<ArrowUpRight size={15} /></Link>{pad && pad.url && <a data-testid="degen-launchpad-open" className="btn-outline" href={pad.url} target={pad.url.startsWith('/') ? '_self' : '_blank'} rel="noreferrer"><Rocket size={15} />Open {pad.name}<ArrowUpRight size={15} /></a>}<div className="degen-chat"><EcosystemChat key={ecosystem.id} ecosystem={{ ...ecosystem, id: `${ecosystem.id}-general` }} /></div><div className="degen-coins-rail" data-testid="degen-coins-rail"><span className="degen-coins-tab"><b>COIN RADAR</b><small>Hover for feeds</small></span><div className="degen-coins-popover"><div className="degen-coins"><TopCoins key={ecosystem.id} ecosystem={ecosystem} /></div></div></div><EcosystemPlatforms ecosystem={ecosystem} /></aside>}</main><WalletModal open={walletOpen} onClose={() => setWalletOpen(false)} />
  </div>;
}