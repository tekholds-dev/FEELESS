...
export default function Terminal() {
  ...
  const assets = useMarket('/assets', 90000); const fee = assets.data?.assets.find(a => a.id === 'fee'); const feeCat = assets.data?.assets.find(a => a.id === 'feecat');
  ...
  const pairs = useMemo(() => {
    ...
    return list;
  }, [market.data, chain, minLiquidity, activePad, tab, kind, query]);
  const newPairs = (newFeed.data?.pairs || []).filter(p => matchesPad(p, activePad));
  useClock(15000);
  ...
  const focus = selected ? <TokenFocus pair={selected} has={has} toggle={toggle} defaultInterval={settings.chartInterval || '1h'} /> : <FeeHeartbeat asset={fee} loading={assets.loading} />;
  return <div className={`terminal-app command-terminal ${settings.compact ? 'compact-rows' : ''} ${settings.reducedMotion ? 'reduced-motion' : ''}`} style={{ '--context-accent': ecosystem.color }}><MouseGlow /><TerminalHeader onWallet={() => setWalletOpen(true)} onMenu={() => setMenuOpen(v => !v)} query={query} /><MarketTicker /><ContextBar />
    <div className="terminal-body"><TerminalSidebar open={menuOpen} onClose={() => setMenuOpen(false)} savedCount={watchlist.length} /><main className="terminal-main" data-testid={`terminal-page-${page || 'home'}`}>
      ...
      {isMarket && <div className={`terminal-content-grid ${!isHome && page !== 'trade' ? 'market-wide' : ''}`}><div className="terminal-primary">
        ...
        {!['', 'trade'].includes(page) && <div className="command-page-title"><span className="eyebrow">{ecosystem.name.toUpperCase()} / ON-CHAIN INTELLIGENCE</span><h1>{query ? 'Follow the contract.' : page === 'new' ? 'Fresh on the radar.' : page === 'pump' ? 'Deep in the trenches.' : page === 'movers' ? 'Read the acceleration.' : 'Find the next rotation.'}</h1><p>{query ? `Provider results for “${query}”` : 'Real signals, within provider coverage. No invented activity.'}</p></div>}
        <ContractScanner />
