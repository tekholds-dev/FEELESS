...
function CreateCat({ onCreated, onCancel }) {
  ...
  const [brainCatalog, setBrainCatalog] = useState([]);
  ...
  const brainInfo = brainCatalog.find(item => item.id === form.brain);
  const selectedAvatar = CAT_VARIATIONS[form.avatar] || CAT_VARIATIONS[0];
  useEffect(() => { api('/api/cats/brains').then(result => setBrainCatalog(result.brains || [])).catch(() => {}); }, []);
  ...
  return <CreateCatWizard form={form} update={update} brain={brain} brainInfo={brainInfo} brainCatalog={brainCatalog} step={step} setStep={setStep} onCancel={onCancel} submit={submit} error={error} saving={saving} />;
}

function BrainCost({ info }) {
  if (!info) return <span className="paper-brain-cost">Checking provider…</span>;
  return <span className="paper-brain-cost">{info.estimatedTokenCostUsd > 0 ? `~$${Number(info.estimatedTokenCostUsd).toFixed(4)} / cycle` : 'No provider cost'}</span>;
}

function CreateCatWizard({ form, update, brain, brainInfo, brainCatalog, step, setStep, onCancel, submit, error, saving }) {
