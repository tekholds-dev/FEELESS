...
export const PumpRadarView = ({ newFeed, trendingFeed, onSelect }) => {
  ...
  const providerError = activeFeed.error || activeFeed.data?.error;
  ...
  const nextRefresh = providerError ? 'RETRYING' : activeFeed.refreshing ? 'SYNCING' : ageSeconds == null ? 'CONNECTING' : `NEXT SYNC ~${Math.max(0, 15 - (ageSeconds % 15))}s`;
  ...
  return <section className="pump-radar" data-testid="pump-radar">
    ...
    {providerError && <p className="pump-radar-error" role="alert" data-testid="pump-radar-error">{providerError} <button type="button" onClick={() => activeFeed.reload()}>Retry</button></p>}
    {!providerError && activeFeed.loading && !stagePairs.length && <p className="truth-empty" role="status" data-testid="pump-radar-loading">Connecting to live provider snapshots…</p>}
    {!providerError && !activeFeed.loading && !stagePairs.length && <div className="truth-empty" data-testid={`pump-radar-${stage}-empty`}>{stageUnavailable ? 'Graduation status is not exposed by the current provider feed.' : stage === 'watchlist' ? 'Star provider-indexed pools to build a personal radar.' : `No ${stage} pools are currently visible in this provider snapshot.`}</div>}
