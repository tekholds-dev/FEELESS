...
export const PumpRadarView = ({ newFeed, trendingFeed, onSelect }) => {
  ...
  const newPairs = useMemo(() => (newFeed.data?.pairs || []).filter(pair => matches(pair) && (pair.marketStage === 'new' || isNewPoolDeal(pair))), [newFeed.data, matches]);
  ...
  return <section className="pump-radar" data-testid="pump-radar">
    <div className="pump-radar-hero"><div><span className="eyebrow"><span className="live-dot" /> PUMP RADAR / LIVE MARKET TAPE</span><h2>See the next rotation<br /><span>before the crowd catches up.</span></h2><p>Provider-backed {pumpLaunchpadFeed ? 'coin' : 'pool'} stages refresh every 15 seconds. Prices and deltas are live snapshots, not a trade signal or a claim of launchpad activity.</p></div><div className="pump-radar-live-orbit" aria-hidden="true"><div /><div /><strong>{stagePairs.length}<small>VISIBLE</small></strong></div></div>
    ...
    <div className="pump-radar-summary"><span><small>VISIBLE POOLS</small><b>{stagePairs.length}</b></span><span><small>LIQUIDITY</small><b>{formatUSD(totalLiquidity)}</b></span><span><small>24H FLOW</small><b>{formatUSD(totalVolume)}</b></span><span><small>{stage === 'graduated' ? 'STATUS OBSERVED' : 'LAST SNAPSHOT'}</small><b>{fetchedAt ? formatTime(feed.data.fetched_at) : '—'}</b></span></div>
