...
import { ShieldCheck, ShieldAlert, Shield, ShieldQuestion, Search, Layers, Star, HelpCircle } from 'lucide-react';
import { useWorkspace } from '../../hooks/useWorkspace';
import { fetchLeaderboard, fetchCreator, BADGE_LABEL } from '../../lib/reputation';
  ...
  return <div className="reputation-center" data-testid="reputation-center">
    ...
    <section className="reputation-mechanics" data-testid="reputation-mechanics">
      ...
      <div className="reputation-mechanics-grid">
        ...
        <article><span className="eyebrow">SCORE, 0–100</span><p>Starts at 50. +3 per token tracked (capped at 10), +8 per token that survives 3+ days with liquidity intact (capped at 6), +3 per token with renounced mint authority (capped at 6). Any confirmed rug caps the whole score hard: −40 per rug, capped so one flag can sink a wallet regardless of past good launches.</p></article>
        ...
        <article><span className="eyebrow">BADGES</span><p><b>Flagged</b> — any rug on record, regardless of anything else. <b>Trusted</b> — 3+ tokens with zero rugs, or at least one that's sustained 3+ days. <b>Building</b> — clean history, still short. <b>Unproven</b> — no observations yet; not a penalty, just no data.</p></article>
    ...
    <div className="reputation-top-tabs"><button type="button" className={tab === 'leaderboard' ? 'active' : ''} onClick={() => setTab('leaderboard')}><Layers size={14} />Leaderboard</button><button type="button" className={tab === 'watchlist' ? 'active' : ''} onClick={() => setTab('watchlist')}><Star size={14} />My Watchlist</button></div>
    ...
    <section className="reputation-leaderboard">
